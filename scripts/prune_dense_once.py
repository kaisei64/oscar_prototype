...
de_mask = np.ones(weight_matrix.shape)
for count in range(2):
    print(f'\nweight pruning: {count}')
    if count != 0:
        with torch.no_grad():
            # prune weights that lead to prototypes
            weight_matrix[0, :], de_mask[0, :] = 0, 0
            # weight_matrix[1, :], de_mask[1, :] = 0, 0
            # weight_matrix[2, :], de_mask[2, :] = 0, 0
            # weight_matrix[3, :], de_mask[3, :] = 0, 0
            # weight_matrix[4, :], de_mask[4, :] = 0, 0
            # weight_matrix[5, :], de_mask[5, :] = 0, 0
            # weight_matrix[6, :], de_mask[6, :] = 0, 0
            # weight_matrix[7, :], de_mask[7, :] = 0, 0
            # weight_matrix[8, :], de_mask[8, :] = 0, 0
            # weight_matrix[9, :], de_mask[9, :] = 0, 0
            # prune negative and positive weights
            # de_mask = np.where(weight_matrix.cpu().detach().numpy() > 0, de_mask, 0)
...
pruningtestacc_vis(f'./result/png/prototype_{prototype}/testacc_afterprune.png', 2, learning_history)
