...
for epoch in range(num_epochs):
    ...
    for i, (images, labels) in enumerate(train_loader):
        elastic_images = batch_elastic_transform(images.reshape(-1, in_height * in_width), sigma=sigma, alpha=alpha,
                                                 height=in_height, width=in_width).reshape(-1, in_channel_num, in_height, in_width)
    ...
    if epoch % save_step == 0 or epoch == num_epochs - 1 or (epoch > 200 and epoch % final_pruning_step == 0):
        with torch.no_grad():
            ...
            f_width = int(math.sqrt(len(net.prototype_feature_vectors[0]) / class_num))
            f_height = int(math.sqrt(len(net.prototype_feature_vectors[0]) / class_num))
    ...
    with torch.no_grad():
        ...
        if epoch != 0 and ((epoch <= 200 and epoch % pruning_step == 0) or (epoch > 200 and epoch % final_pruning_step == 0)):
            ...
            if epoch <= 200:
                ...
                proto_matrix *= torch.tensor(proto_mask, device=device, dtype=dtype)
            # remodel step
            # auto-encoder takeover
            original_encoder_conv_list = [module for module in net.modules() if isinstance(module, nn.Conv2d)]
            original_decoder_conv_list = [module for module in net.modules() if isinstance(module, nn.ConvTranspose2d)]
            tmp_encoder, tmp_decoder = list(), list()
            for conv, deconv in zip(original_encoder_conv_list, original_decoder_conv_list):
                tmp_encoder.append(conv.weight.data.clone())
                tmp_decoder.append(deconv.weight.data.clone())
            # prototype feature vector and classifier takeover
            original_prototype_feature = net.prototype_feature_vectors.cpu().detach().numpy().copy()
            original_classifier_weight = net.classifier[0].weight.cpu().detach().numpy().copy()
            pruned_prototype = torch.sum(net.prototype_feature_vectors, dim=1)
            prune_list = list()
            for i, value in enumerate(pruned_prototype):
                if abs(value) < 1e-5:
                    prune_list.append(i)
            original_prototype_feature = np.delete(original_prototype_feature, prune_list, 0)
            original_classifier_weight = np.delete(original_classifier_weight, prune_list, 1)
            # initialize the model
            if epoch <= 200:
                prototype_num -= 10
            elif epoch > 200:
                prototype_num -= 1
            net = ProtoNet(prototype_num).to(device)
            optimizer = optim.Adam(net.parameters(), lr=0.002)
            weight_matrix = net.classifier[0].weight
            de_mask = torch.ones(weight_matrix.shape)
            proto_matrix = net.prototype_feature_vectors
            ...
            prune_proto_list = list()
            # auto-encoder takeover
            new_encoder_conv_list = [module for module in net.modules() if isinstance(module, nn.Conv2d)]
            new_decoder_conv_list = [module for module in net.modules() if isinstance(module, nn.ConvTranspose2d)]
            for ori_conv, new_conv in zip(tmp_encoder, new_encoder_conv_list):
                new_conv.weight.data = ori_conv.to(device)
            for ori_deconv, new_deconv in zip(tmp_decoder, new_decoder_conv_list):
                new_deconv.weight.data = ori_deconv.to(device)
            # prototype feature vector and classifier weight takeover
            net.prototype_feature_vectors.data = torch.from_numpy(original_prototype_feature).to(device)
            net.classifier[0].weight.data = torch.from_numpy(original_classifier_weight).to(device)
